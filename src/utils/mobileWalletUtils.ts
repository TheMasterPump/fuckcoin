...
export const detectMobileWallets = () => {
  if (typeof window === "undefined") return { phantom: false, metamask: false, inApp: { phantom: false, metamask: false }, currentApp: null };
  
  const ua = navigator.userAgent.toLowerCase();
  
  // Détection via User Agent (in-app browsers)
  const isPhantomApp = ua.includes('phantom') || ua.includes('phantom-ios');
  const isMetaMaskApp = ua.includes('metamask');
  
  // Détection via les objets injectés
  ...
  // Détermine dans quelle app on est actuellement
  let currentApp = null;
  if (isPhantomApp) currentApp = 'phantom';
  else if (isMetaMaskApp) currentApp = 'metamask';
  
  return {
    phantom: hasPhantom || isPhantomApp,
    metamask: hasMetaMask || isMetaMaskApp || (isMobile() && hasEthereumProvider),
    inApp: {
      phantom: isPhantomApp,
      metamask: isMetaMaskApp
    },
    currentApp, // 'phantom', 'metamask', ou null
    canConnectBoth: !isPhantomApp && !isMetaMaskApp // true si on est dans un navigateur normal
  };
};