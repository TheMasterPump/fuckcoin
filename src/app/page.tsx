...
import { isMobile, detectMobileWallets } from "@/utils/mobileWalletUtils";
...
const handleSolanaWalletSelection = async (
  ...
  try {
    ...
    if (e?.message?.includes('WalletNotReadyError')) {
      showToastMessage(`${walletName} wallet not ready. Please open the ${walletName} app first.`, "warning");
    } else if (e?.message?.includes('WalletNotSelectedError')) {
      showToastMessage(`Please select ${walletName} wallet.`, "warning");
    } else {
      showToastMessage(`${walletName} connection failed. Make sure the ${walletName} app is installed.`, "error");
    }
  }
};
