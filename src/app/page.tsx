...
// import OftQuote from "@/components/OftQuote"; // Unused
...
// Fonction pour ouvrir le wallet puis revenir au navigateur
const openWalletApp = (walletName: string) => {
  ...
  switch (walletName.toLowerCase()) {
    case 'phantom':
      // Universal link qui ouvre Phantom puis revient
      appUrl = `https://phantom.app/ul/browse/${encodedUrl}?ref=${encodedUrl}`;
      break;
    case 'metamask':
      // Universal link MetaMask
      appUrl = `https://metamask.app.link/dapp/${window.location.hostname}`;
      break;
    default:
      return false;
  }
  
  // Ouvrir dans une nouvelle fenêtre/tab qui se fermera
  const popup = window.open(appUrl, '_blank', 'width=400,height=600');
  
  // Fermer le popup après 3 secondes (l'app sera ouverte)
  setTimeout(() => {
...
// Fonction SIMPLIFIÉE pour la sélection de wallet Solana
const handleSolanaWalletSelection = async (
  ...
  // Essayer directement la connexion, sans vérifications compliquées
  try {
    ...
    // Sur mobile, essayer d'ouvrir l'app
    if (isMobile()) {
      showToastMessage(`Opening ${walletName} app...`, "warning");
      const opened = openWalletApp(walletName);
      if (!opened) {
        showToastMessage(`${walletName} app not available. Please install ${walletName}.`, "error");
      }
    } else {
      // Messages d'erreur desktop
      if (e?.message?.includes('WalletNotReadyError')) {
        showToastMessage(`${walletName} wallet not ready. Please install ${walletName}.`, "warning");
      } else if (e?.message?.includes('WalletNotSelectedError')) {
        showToastMessage(`Please select ${walletName} wallet.`, "warning");
      } else {
        showToastMessage(`${walletName} connection failed. Make sure ${walletName} is installed.`, "error");
      }
    }
  }
};
          ...
          <div className="flex-1 flex items-center bg-[#23242b] border border-[#293244] rounded-xl p-3 text-white font-bold gap-2 relative">
            ...
            {showWalletOptions && !solAccount && (
              <div className="absolute top-full mt-2 bg-[#181a20] border border-[#293244] rounded-xl shadow-lg p-2 flex flex-col z-50 min-w-[150px]">
                ...
                >
                  <span>👻</span> Phantom
                  {isMobile() && <span className="text-xs text-gray-400">(open in Phantom app)</span>}
                </button>
                <button
                  className="px-4 py-2 hover:bg-[#222936] text-left flex items-center gap-2"
                ...
                >
                  <span>🎒</span> Backpack
                  {isMobile() && <span className="text-xs text-gray-400">(open in Backpack app)</span>}
                </button>
                <button
                  className="px-4 py-2 hover:bg-[#222936] text-left flex items-center gap-2"
                ...
                >
                  ...
                  {isMobile() && <span className="text-xs text-gray-400">(open in Solflare app)</span>}
                </button>
              </div>
            )}
          </div>
