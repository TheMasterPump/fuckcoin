...
// Fonction améliorée pour la sélection de wallet Solana - SANS redirection
const handleSolanaWalletSelection = async (
  ...
  if (isMobile()) {
    ...
    if (!walletAvailable) {
      ...
      showToastMessage(`${walletName} not available. Install ${walletName} app or use a different browser.`, "warning");
      return;
    }
  }
  ...
  try {
    ...
    showToastMessage(`${walletName} connection failed. Try refreshing or use a different browser.`, "error");
  }
};
...
export default function Home() {
  ...
  // Détection des wallets disponibles sur mobile
  const [mobileWalletInfo, setMobileWalletInfo] = useState<{phantom: boolean, metamask: boolean, currentApp: string | null}>({
    phantom: false,
    metamask: false, 
    currentApp: null
  });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center py-4 px-4">
      ...
      {/* Indicateur de wallets disponibles sur mobile */}
      {isMobile() && mobileWalletInfo.currentApp && (
        <div className="mb-4 px-4 py-2 bg-blue-600/20 border border-blue-500/30 rounded-lg text-center text-sm text-blue-300">
          📱 In {mobileWalletInfo.currentApp === 'phantom' ? 'Phantom' : 'MetaMask'} app
          {mobileWalletInfo.currentApp === 'phantom' && !mobileWalletInfo.metamask && (
            <div className="text-xs text-gray-400 mt-1">MetaMask not available in this app</div>
          )}
          {mobileWalletInfo.currentApp === 'metamask' && !mobileWalletInfo.phantom && (
            <div className="text-xs text-gray-400 mt-1">Phantom not available in this app</div>
          )}
        </div>
      )}
