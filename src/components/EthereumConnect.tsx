...
const EthereumConnect: React.FC<EthereumConnectProps> = ({
  ...
  const connectWallet = async () => {
    try {
      ...
      // Sur mobile, vérifier si MetaMask est disponible
      if (isMobile()) {
        const wallets = detectMobileWallets();
        if (!wallets.metamask) {
          // Ne PAS rediriger, informer l'utilisateur
          throw new Error("MetaMask not available. Install MetaMask app or use a different browser.");
        }
      }
      ...
      if (e?.message === "MOBILE_WALLET_NOT_DETECTED") {
        setError("MetaMask not detected. Install MetaMask app or use a different browser.");
      } else {
