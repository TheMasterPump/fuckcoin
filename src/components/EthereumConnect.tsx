...
const EthereumConnect: React.FC<EthereumConnectProps> = ({
  ...
  const getProvider = () => {
    ...
    if (!eth) throw new Error("Aucun wallet Ethereum détecté.");
    
    // Sur mobile, on accepte n'importe quel provider ethereum au lieu de vérifier isMetaMask
    const isMobile = typeof window !== "undefined" && 
      (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ||
       window.innerWidth <= 768);
    
    if (!isMobile && !eth.isMetaMask) {
      throw new Error("MetaMask non détecté sur desktop.");
    }
  ...
  const switchOrAddChain = async () => {
    ...
    if (!netCfg) throw new Error("Réseau EVM invalide.");
