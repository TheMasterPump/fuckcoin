...
const EthereumConnect: React.FC<EthereumConnectProps> = ({
  ...
  const connectWallet = async () => {
    try {
      ...
      // Sur mobile, si MetaMask pas détecté, essayer d'ouvrir l'app
      if (isMobile() && errorMsg.includes('No Ethereum wallet detected')) {
        setError("Opening MetaMask app...");
        
        // Ouvrir MetaMask app
        const appUrl = `https://metamask.app.link/dapp/${window.location.hostname}`;
        ...
        setTimeout(() => {
          ...
          setError("Please connect in MetaMask app, then return here");
        }, 3000);
      } else {
        setError(errorMsg);
      }
